    ...
    <div>
      ...
      <ul>{/* will make a list with */}
        {Object.keys(posts).map((id, index) => { //Object.key(posts) return an array with the keys of an object, then with .map, run through each element and return an item list with the associated index
          return <li key={index}>
            <Link href={`/blog2/${id}`}>{posts[id].title}</Link> {/* id is the name of the key of the json object */}
          </li>
        })}
        <li>
          <Link href='/blog2/xd'>This post actually dosent exist but it wont create it and send an 404 error</Link>
        </li>
      </ul>
    </div>
  )
