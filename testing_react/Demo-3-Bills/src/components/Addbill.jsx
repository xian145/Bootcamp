      ...
      <div className='bg-white rounded shadow-lg p-6 m-4 w-full lg:w-3/4 sm:max-w-lg'>
        <div className='mb-4'>
          ...
          <div className='flex mt-4'>
            ...
            />
            <select 
            className='border border-solid border-gray-400 rounded-md hover:border-green-400 cursor-pointer'
            onChange={(event) => setCategory(event.target.value)}> {/* on change gonna put all the option bringed from categories */}
              {props.categories
            ...
            <div className='mt-2 ml-1'>
              <DatePicker 
              className='border border-solid border-gray-400 rounded-md hover:border-green-400 cursor-pointer'
              selected={date} onChange={handleChangeDate} /> {/* this will show the date when we introduce the amount */}
            </div>
